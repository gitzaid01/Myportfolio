...
export default function Header() {
  ...
  useEffect(() => {
    const updateTime = () => {
      setIndiaTime(getTime("Asia/Kolkata", "New Delhi"));
      setUsTime(getTime("America/New_York", "New York"));
    };
    ...
    <header className="text-white shadow-md bg-black fixed top-0 left-0 w-full z-50">
      <nav className="mx-auto flex justify-between items-center py-4 px-4 sm:px-6">
        ...
        <div className="flex items-center flex-wrap">
          <Link
            href="/"
            className="text-5xl font-extrabold mr-2 whitespace-nowrap"
          >
            ZAID
          </Link>
          <p className="text-white font-bold text-[8px] sm:text-[10px] tracking-tighter max-w-[170px] sm:max-w-[200px]">
            &quot;He who has a why to live can bear almost any how, for purpose
        ...
        {/* Desktop Clocks */}
        <div className="hidden md:flex text-md font-light space-x-6">
          <span>{indiaTime}</span>
          <span>{usTime}</span>
        </div>
          ...
          <ul className="flex flex-col mt-10 text-2xl font-semibold">
            <li>
              <Link
                ...
                className="hover:text-gray-400 font-extrabold text-5xl tackling-tighter"
              >
                Approach
              </Link>
            </li>
